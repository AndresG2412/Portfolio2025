...
export default function FormContact() {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send('service_0035deh', 'template_r0as79w', templateParams, 'nctrpObNBSEbwL1cc')
      .then((response) => {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col md:px-24 px-6'>
      ...
      <label className='text-center text-2xl py-3'>Escribe tu Mensaje:</label>
      <textarea
        placeholder="Tu mensaje aquí..."
        value={message}
        onChange={(e) => setMessage(e.target.value)}
        className='border-2 dark:border-white border-black rounded px-4 py-2 resize-none'
      />
